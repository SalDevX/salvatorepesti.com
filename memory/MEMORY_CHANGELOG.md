...
| feat: 3b02a6b — bin/commit gains push_and_deploy() (git push → npx wrangler pages deploy . --branch master); --skip-deploy flag added; solves stale cache on production after every commit | bin/commit |
| memory-keeper: 3b02a6b — tooling-agent.prompt: bin/commit added to OWNERSHIP block + COMMON TASKS; MEMORY.md already current (bin/commit fix at line 69, 70-line budget); MEMORY_MAP.md already current (push_and_deploy() at line 80); graphify update run | agents/tooling-agent.prompt |
| content: aad7983 — Case 01 desc refreshed — full client name "Marta Howell Jewellery", description rewritten; "HSTS preload-listed" + "Mozilla Observatory 130/100" stack chips added (archived from MEMORY.md) | index.html |
| content: f4bff3c — C1 refine — removed inline "HSTS preload-listed · Hardened browser security architecture." from Case 01 paragraph (redundant with stack chips already present) | index.html |
| memory-keeper: f4bff3c — MEMORY.md: archived oldest row (Case 01 desc refresh), added C1 refine entry (70 lines); MEMORY_MAP.md unchanged (no new functions); graphify update run | memory/MEMORY.md, memory/MEMORY_CHANGELOG.md |
| memory-keeper: 3cf5a6f — MEMORY.md: archived 3 stale rows, added Case 02 Holography + removed_lines_for_file() + email rule scoping (70 lines); MEMORY_MAP.md: Holography Case 02 row added, reel-engine→Case 03, removed_lines_for_file() added to Community 8, _rule_email_protection_intact() 4→5 edges; AGENTS.md: Case 02→03 rename, _rule_sftp_deploy_logic_gate() 4→5, _rule_email_protection_intact() 4→5, graph source 100→108 nodes / 146→155 edges; graphify update run (108 nodes, 155 edges, 8 communities); 25/25 smoke tests pass; tools/refresh_god_nodes.py does not exist — god nodes patched manually | AGENTS.md, memory/, graphify-out/ |
| memory-keeper: aeaa931 — MEMORY.md: archived oldest row (Case 02 Holography insert), added Case 01 130/100 entry (70 lines); MEMORY_MAP.md: Community 5 Case 01 notes updated (HSTS preload-listed, Mozilla Observatory 130/100); no new functions; graphify update run | memory/MEMORY.md, memory/MEMORY_MAP.md |
