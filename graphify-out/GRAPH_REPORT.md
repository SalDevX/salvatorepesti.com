...
5. `_rule_sftp_deploy_logic_gate()` - 5 edges
6. `_rule_email_protection_intact()` - 5 edges
7. `Cloudflare Ecosystem` - 5 edges
8. `files_in_diff()` - 4 edges
9. `files_touched_in_diff()` - 4 edges
10. `added_lines_for_file()` - 4 edges
...
- `test_metacontroller_escalates_to_human_on_rule2()` --calls--> `MetaController`  [INFERRED]
  tests/smoke_test.py → tools/meta_controller.py
- `_rule_email_protection_intact()` --calls--> `removed_lines()`  [INFERRED]
...
Cohesion: 0.1
Nodes (26): added_lines(), files_in_diff(), files_touched_in_diff(), _load_project_rules(), main(), Desktop UI changes must be paired with a matching mobile change in the same diff, Asset filenames referenced in diffs must exist on disk in assets/., Desktop UI changes must be paired with a matching mobile change in the same diff (+18 more)
